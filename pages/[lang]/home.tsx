...
import { getTotalSupply, getQuote, burgerGasPerNeoPerSecond } from '@/resources/utils/api/rpcApi'
...
const Home: NextPage = () => {
	...
	const exchangeRate = useMemo(() => {
		if (totalSupply && quoteArr?.[0]) {
			return `$ ${formatNumber(new BigNumber(integerToDecimal(totalSupply, 8)).times(quoteArr[0]).toString(), {
				decimals: 2,
			})}`
		} else {
			return `$ -`
		...
		<div>
			<div className={style.topBackground}>
				<section className={style.topSection}>
					<div>
						...
						<div className={style.bubbleWrap}>
							<div className={style.bubbleContent}>
								...
								<div className={style.bubbleText}>
									{totalSupply ? formatNumber(integerToDecimal(totalSupply, 8)) : '-'}
								</div>
						...
						<div className={`${style.bubbleWrap} ${style.hideInPC} ${style.marginBottom50}`}>
							<div className={style.bubbleContent}>
								<div className={style.bubbleTitle}>APR</div>
								<div className={style.bubbleText}>{apr}</div>
							</div>
			...
			<main className={style.main} id='main'>
				<div className={style.operateWrap} ref={operateRef}>
					...
					<BurgerStation className={operateType !== 'burgerStation' ? style.hideInMobile : ''} />
					<JazzUp className={operateType !== 'jazzUp' ? style.hideInMobile : ''} />
				</div>
				<section className={style.section}>
					...
					<div className={style.sectionCard}>
						...
						<div className={`${style.sectionContent} ${style.marginLeft200}`}>
							<strong className={style.sectionStrongText}>{t('whatExplain')}</strong>
							{router.locale === 'zh' ? (
								<strong className={style.sectionStrongText}>{t('whatExplain1')}</strong>
							) : null}
							<div className={style.marginBottom24}>
								<strong className={style.sectionMiniStrongText1}>{t('contractAddress')}</strong>
								<p className={style.sectionText}>NPmdLGJN47EddqYcxixdGMhtkr7Z5w4Aos</p>
							</div>
							<div>
								<strong className={style.sectionMiniStrongText1}>{t('scriptHash')}</strong>
								<p className={style.sectionText}>0x48c40d4666f93408be1bef038b6722404d9a4c2a</p>
							</div>
						</div>
					</div>
				</section>
				<section className={style.section}>
					<h1 className={`${style.sectionTitle} ${style.lineHeight40}`}>{t('why')}</h1>
					<div className={style.sectionCard}>
						...
						<div className={`${style.sectionContent} ${style.marginRight150}`}>
							<strong className={style.sectionStrongText}>1 NEO = 1.00000000 bNEO</strong>
							<p className={`${style.sectionText} ${style.marginBottom24}`}>{t('whyExplain1')}</p>
				...
				<section className={`${style.section} ${style.textAlignCenter}`}>
					<h1 className={`${style.sectionTitle} ${style.lineHeight40}`}>{t('where')}</h1>
					...
					<strong className={style.sectionMiniStrongTextCenter}>{t('whereExplain2')}</strong>
					<div className={style.sectionImgWrapperPC}>
						{t('language') === 'en' ? (
							<Image src={connection} alt='connection' />
						) : (
				...
				<section className={style.section}>
					<div className={`${style.sectionCard} ${style.hideInMobile}`}>
						<div className={style.sectionContentMiddle}>
							...
							<strong className={style.sectionMiniStrongText} onClick={() => jumpToOperate()}>
								{t('howExplain1')}
